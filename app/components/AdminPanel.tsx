...
export const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  const [activeSection, setActiveSection] = useState<'overview' | 'analytics' | 'shifts' | 'users' | 'leaves' | 'activity' | 'coverage' | 'recommendations' | 'config'>('overview');
    ...
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-wrap items-center justify-center gap-2 pb-6 border-b border-zinc-200">
        <div className="flex flex-wrap bg-zinc-100 p-1 rounded-2xl border border-zinc-200 justify-center gap-1">
          ...
          >
            <BrainCircuit size={16} strokeWidth={activeSection === 'overview' ? 2.5 : 2} />
            Overview
          </button>
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-black transition-all duration-300 ${activeSection === 'recommendations'
              ? 'bg-white text-blue-600 shadow-sm ring-1 ring-zinc-200'
              : 'text-zinc-500 hover:text-zinc-900'
              }`}
          >
            <TrendingUp size={16} strokeWidth={activeSection === 'recommendations' ? 2.5 : 2} />
            Suggestions
          </button>
